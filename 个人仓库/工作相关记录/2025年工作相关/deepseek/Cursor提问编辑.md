...
好的现在我需要你删除DataEditViewModel中的TestResults。并将所有的引用都使用AllChannels来实现。请在删除前通过所有程序中的映射关系确保AllChannels已经拥有了其与TestResults的属性交集，从而不回导致错误的发生。同时将dataeditview界面中的绑定也修改为AllChannels。且要保证功能与现有功能相同。请在修改前查看所有代码了解其中的关系并在修改后编译代码确认没有错误以及逻辑上的问题
