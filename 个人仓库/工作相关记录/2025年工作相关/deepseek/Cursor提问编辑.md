...
我现在将TestResults注释掉了，同时将从ExcelPointData向TestResults中映射点位的地方也注释掉了。将所有的映射关系都迁移至AllChannels中了，现在请你查看整个程序中所有使用到TestResults的地方都会报错。我需要你使用AllChannels来替换掉以前使用TestResults数据的地方如果是方法中需要传入TestResults的则分量中情况，方法的入参只需要TestResults就将其替换为AllChannels。如果方法的入参同时需要TestResults和AllChannels则修改接口和方法使其只传入一个AllChannels就可以完成以前的功能所以请你仔细的阅读代码并对涉及到的接口和实现类中的方法都进行修改，确保没有任何遗漏。修改完成后编译确认没有错误和逻辑上的错误
